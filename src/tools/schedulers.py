...
__all__ = ["multistep_lr", "cosine_lr", "constant_lr", "get_policy", "exp_lr" , "onecycle_lr"]
...
def cosine_lr(args, batch_num):
    ...
    def _lr_adjuster(epoch):
        if epoch < args.warmup_length:
            lr = _warmup_lr(args.warmup_lr, args.base_lr, args.warmup_length, epoch)
        elif epoch <args.epoch_end:
            e = epoch - args.warmup_length
            es = args.epoch_end - args.warmup_length
...
def onecycle_lr(args, batch_num):
    ...
    def _lr_adjuster(epoch):
        if epoch < args.epochs*0.4:
            lr = args.base_lr/(args.epochs*0.4)*epoch
        elif epoch < args.epochs*0.8:
            lr = args.base_lr-args.base_lr*0.95/(args.epochs*0.4)*(epoch-args.epochs*0.4)
        else:
            lr = args.base_lr/20-args.base_lr/20/(args.epochs*0.2)*(epoch-args.epochs*0.8)
